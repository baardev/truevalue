...
\title{The Tholonic Model as a Candidate Alternative to the Standard Model:\\
\large Structural Derivations and Parameter Reduction}
\author{J. W. Milton\\[4pt]{\small Clarity Coalition}}
\date{\small 19 May 2026 \\ v1.0}

\begin{document}
\maketitle

\noindent\textbf{Keywords:} tholonic model, Standard Model, N-D-C triad, fine structure constant, fermion generations, color charge, quark confinement, Koide formula, parameter reduction, unified field theory, falsifiability
...
3}), derive five classical mathematical constants from the triadic
recursion
...
1}), and show that the atom's measurable properties, including quark
occupancy structure, color charge, the virial theorem, tetrahedral
orbital geometry, and the Koide formula's unexplained \(2/3\) lepton
mass ratio, are structural predictions of the tholonic framework rather
than independent empirical accidents
...
8}). This paper synthesizes those results into a direct comparison with
the Standard Model. The tholonic framework addresses five of the
Standard Model's unexplained structural facts (three fermion
...
\subsection{The Standard Model's achievements}\label{the-standard-models-achievements}
...
\subsection{The Standard Model's structural limitations}\label{the-standard-models-structural-limitations}
...
\subsection{The criterion for a better theory}\label{the-criterion-for-a-better-theory}
...
\section{The Tholonic Framework: Summary of Prior Results}\label{the-tholonic-framework-summary-of-prior-results}

\subsection{The core claim}\label{the-core-claim}
...
\href{https://github.com/baardev/truevalue/blob/main/docnav/Research/papers/3_minimal-recursive-triadic-framework/3_minimal-recursive-triadic-framework.pdf}{Paper
3 of this series} proves formally that three roles are the minimum for
non-trivial convergent recursion. A system with fewer than three roles
is either trivially stable (constant, no dynamics) or divergent (no
stable equilibrium). The proof is constructive and does not depend on
any physical assumption.

\subsection{N-D-C role assignments and the thologram}\label{n-d-c-role-assignments-and-the-thologram}
...
tholographic seeds for the \(\pi/4\) branch derived in
\href{https://github.com/baardev/truevalue/blob/main/docnav/Research/papers/1_recursive-tholonic-five-constants/1_recursive-tholonic-five-constants.pdf}{Paper
1}.
...
\subsection{Results from the paper series}\label{results-from-the-paper-series}
...
\item
  ...
  1}:} Five classical constants (\(\pi/4\), \(\phi\), \(e\),
  \(\sqrt{2}\), \(\ln 2\)) emerge as limits of the tholonic recursive
  system from its own internal geometry, without separate derivations
  for each.
\item
  ...
  3}:} Three functional roles are the proved minimum for non-trivial
  convergent recursion. Any fewer roles produces either a constant
  system or a divergent one.
\item
  \textbf{\href{https://github.com/baardev/truevalue/blob/main/docnav/Research/papers/6_qualitative-nature-integers-triadic-roles/6_qualitative-nature-integers-triadic-roles.pdf}{Paper
  6}:} The integers 1, 2, and 3 have qualitatively distinct structural
  roles in minimal recursive systems, providing a formal basis for the
  N-D-C assignment.
\item
  \textbf{\href{https://github.com/baardev/truevalue/blob/main/docnav/Research/papers/7_cambridge-semantics-tholonic-framework/7_cambridge-semantics-tholonic-framework.pdf}{Paper
  7}:} The N-D-C framework has been independently rediscovered by
  Cambridge Semantics through two decades of production engineering of
  large-scale knowledge graphs, providing an empirical convergence that
  is not constructed from tholonic premises.
\item
  \textbf{\href{https://github.com/baardev/truevalue/blob/main/docnav/Research/papers/8_atom-as-tholon-measurable-phenomena/8_atom-as-tholon-measurable-phenomena.pdf}{Paper
  8}:} The atom is measurably a tholon. Six specific correspondences are
  established, graded by strength, and compared with the Standard
  Model's explanatory coverage.
\end{itemize}

\begin{center}\rule{0.5\linewidth}{0.5pt}\end{center}

\section{Structural Axiom Reduction}\label{structural-axiom-reduction}

\subsection{The three-role minimality theorem}\label{the-three-role-minimality-theorem}

\href{https://github.com/baardev/truevalue/blob/main/docnav/Research/papers/3_minimal-recursive-triadic-framework/3_minimal-recursive-triadic-framework.pdf}{Paper
3's} central result is that the N-D-C triad is not an arbitrary choice
of three roles but the unique minimum structure for convergent recursive
self-organization. The theorem states: any recursive system that
achieves non-trivial stable convergence requires at least three distinct
functional roles in the N-D-C pattern. Systems with fewer roles either
do not converge (no D to bound C) or converge trivially to a constant
(no C to integrate). Systems with more than three roles are either
...
\subsection{Three color charges as a tholonic structural prediction}\label{three-color-charges-as-a-tholonic-structural-prediction}
...
\subsection{Three fermion generations as a structural prediction}\label{three-fermion-generations-as-a-structural-prediction}
...
The tholonic prediction follows directly from
\href{https://github.com/baardev/truevalue/blob/main/docnav/Research/papers/3_minimal-recursive-triadic-framework/3_minimal-recursive-triadic-framework.pdf}{Paper
3's} minimality theorem: if the three-role requirement applies at the
generation level of the fermion spectrum (each generation playing one of
the N, D, C roles in the fermion tholon), then exactly three generations
is a structural necessity, not a numerical coincidence. The N generation
...
\subsection{Quark confinement as incomplete-tholon instability}\label{quark-confinement-as-incomplete-tholon-instability}
...
\subsection{Three spatial dimensions as a tholonic constraint}\label{three-spatial-dimensions-as-a-tholonic-constraint}
...
\section{Quantitative Parameter Derivations}\label{quantitative-parameter-derivations}

\subsection{The fine structure constant: leading candidate}\label{the-fine-structure-constant-leading-candidate}
...
with logarithmic (integrating) convergence
(\href{https://github.com/baardev/truevalue/blob/main/docnav/Research/papers/1_recursive-tholonic-five-constants/1_recursive-tholonic-five-constants.pdf}{Paper
1}, Class B branch). The D role is the bounding component; \(e\) raised
to the D-axis power sets the overall magnitude. Whether this structural
reading can be derived from role axioms rather than observed numerically
is the central open theoretical question.
...
\subsection{The proton/electron mass ratio}\label{the-protonelectron-mass-ratio}
...
atom
(\href{https://github.com/baardev/truevalue/blob/main/docnav/Research/papers/8_atom-as-tholon-measurable-phenomena/8_atom-as-tholon-measurable-phenomena.pdf}{Paper
8}, Section 4), the proton occupies the D role, the electron occupies
the C role, and the stable orbital (N state) emerges from their balance.
The role weights in the 2-3-6 pattern assign N = 6, D = 2, C = 3.
...
from
\href{https://github.com/baardev/truevalue/blob/main/docnav/Research/papers/1_recursive-tholonic-five-constants/1_recursive-tholonic-five-constants.pdf}{Paper
1}) raised to the D-axis power (5). Written with thologram structural
integers:
...
\subsection{The Koide formula: D:C weight ratio and lepton masses}\label{the-koide-formula-dc-weight-ratio-and-lepton-masses}
...
\textbf{The quark triplet constraint.}
\href{https://github.com/baardev/truevalue/blob/main/docnav/Research/papers/8_atom-as-tholon-measurable-phenomena/8_atom-as-tholon-measurable-phenomena.pdf}{Paper
8} (Section 8.1) reports the Koide \(Q\) values for the quark triplets
...
\subsubsection{\texorpdfstring{4.4 The Tsirelson bound: summary of the
\href{https://github.com/baardev/truevalue/blob/main/docnav/Research/papers/8_atom-as-tholon-measurable-phenomena/8_atom-as-tholon-measurable-phenomena.pdf}{Paper
8}
derivation}{4.4 The Tsirelson bound: summary of the Paper 8 derivation}}\label{the-tsirelson-bound-summary-of-the-paper-8-derivation}

\href{https://github.com/baardev/truevalue/blob/main/docnav/Research/papers/8_atom-as-tholon-measurable-phenomena/8_atom-as-tholon-measurable-phenomena.pdf}{Paper
8} (Section 3.4) derives the Tsirelson bound \(2\sqrt{2}\) from tholonic
first principles at two independent levels. The derivation is summarized
here because it constitutes a completed structural derivation of a
previously unexplained quantum bound.

\textbf{From the real-virtual tholon pair.}
\href{https://github.com/baardev/truevalue/blob/main/docnav/Research/papers/1_recursive-tholonic-five-constants/1_recursive-tholonic-five-constants.pdf}{Paper
1} proves that the tholonic recursive system converges to \(\sqrt{2}\)
as one of its five classical limits. A complete tholon always comprises
a real N-D-C configuration and its virtual complement. Both are full
recursive triadic systems and both independently converge to
\(\sqrt{2}\). Their composite value is
\(\sqrt{2} + \sqrt{2} = 2\sqrt{2}\). An entangled particle pair is, in
tholonic terms, a complete real-virtual tholon pair; the maximum
achievable CHSH correlation is bounded by this composite convergence.

\textbf{From pure angle geometry.} The tholonic D-C balance angle is
\(\pi/4\) (the optimal angle where \(\sin\theta = \cos\theta\)), which
is also the \(\pi\)-branch seed angle from
\href{https://github.com/baardev/truevalue/blob/main/docnav/Research/papers/1_recursive-tholonic-five-constants/1_recursive-tholonic-five-constants.pdf}{Paper
1}. At this angle, the maximum CHSH value is:
...
from
\href{https://github.com/baardev/truevalue/blob/main/docnav/Research/papers/3_minimal-recursive-triadic-framework/3_minimal-recursive-triadic-framework.pdf}{Paper
3}, Lemma 4.2, without reference to measurement scenarios or quantum
mechanics. This is the first structural derivation of the CHSH operator
form and Tsirelson value from non-quantum-mechanical first principles.

\begin{center}\rule{0.5\linewidth}{0.5pt}\end{center}

\section{Parameter Comparison: Standard Model vs Tholonic Model}\label{parameter-comparison-standard-model-vs-tholonic-model}
...
\subsection{What remains to be derived}\label{what-remains-to-be-derived}
...
\subsection{Strong falsification conditions}\label{strong-falsification-conditions}
...
\subsection{What would constitute confirmation}\label{what-would-constitute-confirmation}
...
\subsection{The fine structure constant derivation as the priority target}\label{the-fine-structure-constant-derivation-as-the-priority-target}
...
\subsection{Required theoretical work}\label{required-theoretical-work}
...
\subsection{Scope of the claim}\label{scope-of-the-claim}
...
\item
  ...
  \(\phi\), \(e\), \(\sqrt{2}\), \(\ln 2\) from tholonic recursion.
\item
  \textbf{\href{https://github.com/baardev/truevalue/blob/main/docnav/Research/papers/3_minimal-recursive-triadic-framework/3_minimal-recursive-triadic-framework.pdf}{Paper
...
\item
  \textbf{\href{https://github.com/baardev/truevalue/blob/main/docnav/Research/papers/6_qualitative-nature-integers-triadic-roles/6_qualitative-nature-integers-triadic-roles.pdf}{Paper
  6 in this series}:} \emph{The Qualitative Nature of One, Two, and
  Three: Structural Role Assignment in Minimal Recursive Systems.}
  Formal basis for the N-D-C integer role assignment.
\item
  \textbf{\href{https://github.com/baardev/truevalue/blob/main/docnav/Research/papers/7_cambridge-semantics-tholonic-framework/7_cambridge-semantics-tholonic-framework.pdf}{Paper
  7 in this series}:} \emph{Engineering Toward N: Cambridge Semantics as
  Empirical Validation of the Tholonic N-D-C Framework.} Independent
  engineering convergence on the N-D-C structure.
\item
  \textbf{\href{https://github.com/baardev/truevalue/blob/main/docnav/Research/papers/8_atom-as-tholon-measurable-phenomena/8_atom-as-tholon-measurable-phenomena.pdf}{Paper
  8 in this series}:} \emph{The Atom as a Measurable Tholon: From
  Einstein's Incomplete Electron to a Field-First Ontology.} Atomic
  correspondences and Standard Model comparison that this paper extends.
\item
