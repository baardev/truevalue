...
\section{Power-of-Two Convergence Counts in the Tholonic Seed Space: A
Complete Hierarchy of Classical
Constants}\label{power-of-two-convergence-counts-in-the-tholonic-seed-space-a-complete-hierarchy-of-classical-constants}

\textbf{Author:} J. W. Milton, Clarity Coalition

\textbf{Version:} 1.1

\textbf{Date:} 8 June 2026

\textbf{Keywords:} tholonic model, N-D-C triad, five constants, golden
ratio, pi, Euler's number, natural logarithm, seed-space sweep,
convergence hierarchy, generative entropy, power of two, Fibonacci
seeds, archetype
...
1 of this series} establishes that five classical mathematical constants
(\(\pi/4\), \(\varphi\), \(e\), \(\sqrt{2}\), \(\ln 2\)) emerge as
limits of a single family of three-variable tholonic recurrences
parameterized by an initial triple \((N_0, D_0, C_0)\) with canonically
distinct seeds. This paper extends that structural analysis by an
exhaustive computational sweep: all \(4^4 = 256\) combinations of the
four canonical seed values \(\{1, 2, 3, 5\}\) are applied to each of the
five algorithms, yielding 1,280 independent convergence runs.
...
individual series; those are classical and established in
\href{https://github.com/baardev/truevalue/raw/main/docnav/Research/papers/1_recursive-tholonic-five-constants/1_recursive-tholonic-five-constants.pdf}{Paper
1}. The contribution is the combinatorial structure of the seed space,
the entropy interpretation, and the identification of \(\pi\) as an
...
establishes that five classical constants, \(\pi/4\) (via the tholonic
Class A advancing branch, which reduces mathematically to a grouping of
the Leibniz series), \(\varphi\) (via the continued-fraction fixed
point), \(e\) (via the factorial series), \(\sqrt{2}\) (via the
Babylonian method), and \(\ln 2\) (via the alternating harmonic series),
all emerge from a single three-role recursive grammar parameterized by a
tholonic triple \((N, D, C)\). The three roles, Negotiation (\(N\)),
Definition (\(D\)), and Contribution (\(C\)), are functionally
irreducible: they cannot be collapsed to fewer than three even when two
positions share the same numerical seed value.

\href{https://github.com/baardev/truevalue/raw/main/docnav/Research/papers/1_recursive-tholonic-five-constants/1_recursive-tholonic-five-constants.pdf}{Paper
1's} structural contribution is the \emph{grammar}: the five branches
are distinguished by initial seeds and traversal rules, but the role
assignments are invariant. The present paper asks a different question:
if we treat the seed values not as fixed canonical inputs but as a space
to be swept, what does the distribution of convergent outcomes reveal
about the five constants?

\subsubsection{1.2 The sweep question}\label{the-sweep-question}
...
\subsubsection{1.3 Relation to the existing
series}\label{relation-to-the-existing-series}

This paper is a companion to
\href{https://github.com/baardev/truevalue/raw/main/docnav/Research/papers/1_recursive-tholonic-five-constants/1_recursive-tholonic-five-constants.pdf}{Paper
1} and builds on its notation and convergence proofs.
...
establishes the irreducibility of three roles and is the foundational
minimality result.
...
6}
\href{https://github.com/baardev/truevalue/raw/main/docnav/Research/papers/6_qualitative-nature-integers-triadic-roles/6_qualitative-nature-integers-triadic-roles.pdf}{Mil26c}
establishes that the qualitative character of the integers 1, 2, and 3
determines the role assignment. The present paper sits closest to
\href{https://github.com/baardev/truevalue/raw/main/docnav/Research/papers/1_recursive-tholonic-five-constants/1_recursive-tholonic-five-constants.pdf}{Paper
1}: both concern the five constants under tholonic recursion, but
\href{https://github.com/baardev/truevalue/raw/main/docnav/Research/papers/1_recursive-tholonic-five-constants/1_recursive-tholonic-five-constants.pdf}{Paper
1} establishes the structural grammar while this paper explores the
combinatorial topology of the seed space.
...
that the hierarchy is bound to the tholonic axis geometry of
\href{https://github.com/baardev/truevalue/raw/main/docnav/Research/papers/1_recursive-tholonic-five-constants/1_recursive-tholonic-five-constants.pdf}{Paper
1}; the entropy interpretation; the divide-by-four cascade; the
restoration of \(\pi\) from the raw \(\pi/4\) output; and an argument
that the hierarchy reflects the algebraic standing of the constants.

\textbf{What this paper does not provide.} New convergence proofs for
the individual series (those are in
\href{https://github.com/baardev/truevalue/raw/main/docnav/Research/papers/1_recursive-tholonic-five-constants/1_recursive-tholonic-five-constants.pdf}{Paper
1} and the classical literature); any claim that the seed set
\(\{1, 2, 3, 5\}\) is the only set with this property; or a formal
theorem characterizing all seed sets that produce a power-of-two
frequency table.
...
\subsection{2. Algorithms and Seed
Parameterization}\label{algorithms-and-seed-parameterization}

\subsubsection{2.1 The tholonic NDC seeding
model}\label{the-tholonic-ndc-seeding-model}
...
numerical value while playing structurally different roles (see
\href{https://github.com/baardev/truevalue/raw/main/docnav/Research/papers/3_minimal-recursive-triadic-framework/3_minimal-recursive-triadic-framework.pdf}{Paper
3}, Lemma 4.2).

\subsubsection{2.2 The five algorithms}\label{the-five-algorithms}
...
The convergence classification follows
\href{https://github.com/baardev/truevalue/raw/main/docnav/Research/papers/1_recursive-tholonic-five-constants/1_recursive-tholonic-five-constants.pdf}{Paper
1}, Section 4. Each run iterates until successive outputs agree to six
decimal places.
...
\subsection{3. The Sweep}\label{the-sweep}

\subsubsection{3.1 Design}\label{design}
...
\subsection{4. The Power-of-Two
Structure}\label{the-power-of-two-structure}
...
\subsubsection{4.2 Why this is
non-trivial}\label{why-this-is-non-trivial}
...
\subsubsection{4.3 Explanation for the universal
cases}\label{explanation-for-the-universal-cases}
...
\subsubsection{4.4 The exponents descend by
two}\label{the-exponents-descend-by-two}
...
\subsubsection{4.5 Binding to the tholonic axis
geometry}\label{binding-to-the-tholonic-axis-geometry}

\href{https://github.com/baardev/truevalue/raw/main/docnav/Research/papers/1_recursive-tholonic-five-constants/1_recursive-tholonic-five-constants.pdf}{Paper
1}
\href{https://github.com/baardev/truevalue/raw/main/docnav/Research/papers/1_recursive-tholonic-five-constants/1_recursive-tholonic-five-constants.pdf}{Mil26a},
Section 2.1 derives the five algorithms from a geometric thologram in
which six vertices carry successive powers of two:
...
divides the path count by exactly \(4 = 2^2\).
\href{https://github.com/baardev/truevalue/raw/main/docnav/Research/papers/1_recursive-tholonic-five-constants/1_recursive-tholonic-five-constants.pdf}{Paper
1} proves that \(\Delta = \mathrm{istep}^2 = 4\) is the unique Class A
advance step for the \(\pi/4\) branch, derived from the Instantiation
axis multiplier. The cascade rate of the seed-space hierarchy is
numerically the same \(2^2\) that drives parameter advancement in the
\(\pi/4\) algorithm. Whether this is a structural correspondence or a
causal derivation is not proved here: the claim is that the two
quantities are equal and that this equality is consistent with the axis
geometry, not that one forces the other by a theorem already in hand.
...
established in
\href{https://github.com/baardev/truevalue/raw/main/docnav/Research/papers/1_recursive-tholonic-five-constants/1_recursive-tholonic-five-constants.pdf}{Paper
1}: the vertex values \(2^0, 2^1, 2^2\); the axis multipliers
\(5, 3, 2\); the step \(\Delta = \mathrm{istep}^2 = 4\); and the seed
triple \((1, 3, 5)\) for the \(\pi/4\) branch. The following bindings
are argued here on the basis of the sweep data and structural
...
\subsubsection{5.3 Two distinct kinds of
complexity}\label{two-distinct-kinds-of-complexity}
...
\subsubsection{5.4 Uniqueness of the Fibonacci
seeds}\label{uniqueness-of-the-fibonacci-seeds}
...
\subsubsection{6.1 Shared properties}\label{shared-properties}
...
\subsubsection{6.2 A natural polarity}\label{a-natural-polarity}
...
\subsubsection{7.1 The divide-by-four
cascade}\label{the-divide-by-four-cascade}
...
is derived from the Instantiation axis
(\href{https://github.com/baardev/truevalue/raw/main/docnav/Research/papers/1_recursive-tholonic-five-constants/1_recursive-tholonic-five-constants.pdf}{Paper
1}, Section 2.1). The algorithm was not constructed from the Leibniz
series; the Leibniz series is what the algorithm reduces to
mathematically, as a consequence of its structure
(\href{https://github.com/baardev/truevalue/raw/main/docnav/Research/papers/1_recursive-tholonic-five-constants/1_recursive-tholonic-five-constants.pdf}{Paper
1}, Proposition 5.5).
...
\subsubsection{7.3 The complete hierarchy}\label{the-complete-hierarchy}
...
\subsubsection{7.4 The four-stage emanation
structure}\label{the-four-stage-emanation-structure}
...
\subsubsection{8.1 The apparent paradox}\label{the-apparent-paradox}
...
\subsubsection{8.2 The archetype-instance
distinction}\label{the-archetype-instance-distinction}
...
\(2^0, 2^2, \ldots, 2^{2\log_2|S|}\) under the five
\href{https://github.com/baardev/truevalue/raw/main/docnav/Research/papers/1_recursive-tholonic-five-constants/1_recursive-tholonic-five-constants.pdf}{Paper
1} algorithms if and only if \(S\) is the axis-derived vocabulary
...
Recursive Triadic Framework.'' Clarity Coalition preprint, May 2026.
{[}\href{https://github.com/baardev/truevalue/raw/main/docnav/Research/papers/1_recursive-tholonic-five-constants/1_recursive-tholonic-five-constants.pdf}{Paper
1 of this series}.{]}

\href{https://github.com/baardev/truevalue/raw/main/docnav/Research/papers/3_minimal-recursive-triadic-framework/3_minimal-recursive-triadic-framework.pdf}{Mil26b}
Milton, J. W. ``A Minimal Recursive Triadic Framework: Irreducibility
and Structural Necessity.'' Clarity Coalition preprint, April 2026.
{[}\href{https://github.com/baardev/truevalue/raw/main/docnav/Research/papers/3_minimal-recursive-triadic-framework/3_minimal-recursive-triadic-framework.pdf}{Paper
3 of this series}.{]}

\href{https://github.com/baardev/truevalue/raw/main/docnav/Research/papers/6_qualitative-nature-integers-triadic-roles/6_qualitative-nature-integers-triadic-roles.pdf}{Mil26c}
Milton, J. W. ``The Qualitative Nature of One, Two, and Three:
Structural Role Assignment in Minimal Recursive Systems.'' Clarity
Coalition preprint, April 2026.
{[}\href{https://github.com/baardev/truevalue/raw/main/docnav/Research/papers/6_qualitative-nature-integers-triadic-roles/6_qualitative-nature-integers-triadic-roles.pdf}{Paper
6 of this series}.{]}
