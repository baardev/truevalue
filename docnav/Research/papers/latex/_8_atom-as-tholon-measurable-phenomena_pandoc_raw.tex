...
\section{The Atom as a Measurable Tholon: From Einstein's Incomplete
Electron to a Field-First
Ontology}\label{the-atom-as-a-measurable-tholon-from-einsteins-incomplete-electron-to-a-field-first-ontology}
...
\textbf{Keywords:} tholonic model, N-D-C triad, atomic structure,
electron, field ontology, quark charges, virial theorem, Tesla,
Einstein, quantum field theory, unified field theory, Koide formula,
fine structure constant, fermion generations, falsifiability
...
its quantitatively verified properties (quark occupancy structure, the
virial theorem's kinetic/potential energy balance, color charge
structure, tetrahedral orbital geometry, and the Koide formula's
unexplained \(2/3\) lepton mass ratio) are structural predictions of the
tholonic framework rather than independent empirical accidents. The
...
(\(\sqrt{2} + \sqrt{2} = 2\sqrt{2}\), each component proved in
...
1}); and (b) pure angle geometry, where the CHSH maximum equals
...
3's} minimality lemmas without presupposing Hilbert space or quantum
mechanics. Beyond structural correspondence, the paper argues that the
tholonic model is a candidate for a genuinely better theory than the
Standard Model: it reduces the axiom count and derives structurally
several quantities the Standard Model leaves as free parameters. The
Standard Model requires 17 distinct quantum fields and 19 free
parameters. The tholonic model proposes one underlying field and derives
structurally the Koide formula's \(2/3\), the three-generation and
three-color structure, and quark confinement. A systematic search over
...
\subsection{1. The Problem: Why the Electron as Described Is
Insufficient}\label{the-problem-why-the-electron-as-described-is-insufficient}

\subsubsection{1.1 Tesla's Objection}\label{teslas-objection}
...
\subsubsection{1.2 Einstein's Objection}\label{einsteins-objection}
...
\subsubsection{1.3 Quantum Field Theory: A Partial
Vindication}\label{quantum-field-theory-a-partial-vindication}
...
\subsection{2. The Tholonic Model: A Field-First
Ontology}\label{the-tholonic-model-a-field-first-ontology}

\subsubsection{2.1 Core Claim}\label{core-claim}
...
\subsubsection{2.2 The N-D-C Triad}\label{the-n-d-c-triad}
...
(no sustainable recursion).
\href{https://github.com/baardev/truevalue/blob/main/docnav/Research/papers/3_minimal-recursive-triadic-framework/3_minimal-recursive-triadic-framework.pdf}{Paper
3 in this series} proves this irreducibility formally for recursive
systems.

\subsubsection{2.3 The Thologram's Numerical
Patterns}\label{the-tholograms-numerical-patterns}
...
(established in
\href{https://github.com/baardev/truevalue/blob/main/docnav/Research/papers/3_minimal-recursive-triadic-framework/3_minimal-recursive-triadic-framework.pdf}{Paper
3}): N is the unit state, D introduces the first distinction, and C
integrates to the next binary level. The 1,2,4 pattern gives a D:C
vertex ratio of \(2:4 = 1:2\). This is the pattern appropriate when
modeling energy or amplitude ratios in which C carries twice the
structural weight of D.
...
\(C_0 = 5\) in the \(\pi/4\) branch of the tholonic ladder
(\href{https://github.com/baardev/truevalue/blob/main/docnav/Research/papers/1_recursive-tholonic-five-constants/1_recursive-tholonic-five-constants.pdf}{Paper
1 in this series}). This pattern is appropriate when working with the
...
\subsubsection{2.4 The Tholon}\label{the-tholon}
...
\subsection{3. The Atom as a Measurable Tholon: Lines of
Evidence}\label{the-atom-as-a-measurable-tholon-lines-of-evidence}
...
\subsubsection{3.1 Quark Occupancy Structure
(Strong)}\label{quark-occupancy-structure-strong}
...
\subsubsection{3.2 Color Charge, the SU(3) Trigram, and Confinement
(Strong)}\label{color-charge-the-su3-trigram-and-confinement-strong}
...
\subsubsection{3.3 The Virial Theorem as the D-C Balance Condition
(Strong)}\label{the-virial-theorem-as-the-d-c-balance-condition-strong}
...
The stable orbital, the N state, is the negotiated equilibrium at which
the D-C balance satisfies the virial condition.
...
the thologram: the simplest non-arbitrary ratio the roles can take given
their positions in a binary state space.
...
\textbf{Step 1: The tholonic \(\sqrt{2}\) branch.}
\href{https://github.com/baardev/truevalue/blob/main/docnav/Research/papers/1_recursive-tholonic-five-constants/1_recursive-tholonic-five-constants.pdf}{Paper
1 in this series} establishes that the tholonic recursive system (a
three-variable recurrence on \((N, D, C)\)) converges to \(\sqrt{2}\) as
one of its five classical limits. This is the fixed-parameter branch
...
maximized) and corresponds to what the CHSH expression \(S\) measures.
...
follows from
\href{https://github.com/baardev/truevalue/blob/main/docnav/Research/papers/1_recursive-tholonic-five-constants/1_recursive-tholonic-five-constants.pdf}{Paper
1's} recurrence structure (Lemma 3.1), in which N at the next level is a
function of D and C independently, not their sum. The minimal
non-trivial coupling between two functionally independent roles is
bilinear.
...
without D is the virtual tholon contribution
(\href{https://github.com/baardev/truevalue/blob/main/docnav/Research/papers/3_minimal-recursive-triadic-framework/3_minimal-recursive-triadic-framework.pdf}{Paper
3} shows C without D is unstable) \\
\end{longtable}
...
\href{https://github.com/baardev/truevalue/blob/main/docnav/Research/papers/3_minimal-recursive-triadic-framework/3_minimal-recursive-triadic-framework.pdf}{Paper
3}, Lemma 4.2 and Definition 4.3: Definition 4.3 assigns D as the
bounding role and C as the accumulating role. Lemma 4.2 proves that a
system with only C and no D cannot achieve convergence (C without D is
unstable, grows without bound). Therefore the C \(\times\) C
interaction, which has no D from either party, is precisely the
C-without-D condition: it is the virtual tholon (the unstable,
...
\(+1\). T3 is fully derived from
\href{https://github.com/baardev/truevalue/blob/main/docnav/Research/papers/3_minimal-recursive-triadic-framework/3_minimal-recursive-triadic-framework.pdf}{Paper
3's} axioms without reference to measurement scenarios or quantum
mechanics.
...
Alice's D=0° and C=90° span a quarter circle; their bisector is at 45° =
\(\pi/4\), the tholonic D-C balance angle where
\(\sin\theta = \cos\theta\). This is also the \(\pi\)-branch seed angle
from
\href{https://github.com/baardev/truevalue/blob/main/docnav/Research/papers/1_recursive-tholonic-five-constants/1_recursive-tholonic-five-constants.pdf}{Paper
1} (Section 3 of the five-constants derivation). At this optimal angle:
\(\mathcal{C}(\pi/4) = \cos(\pi/4) = 1/\sqrt{2}\), the tholonic
convergence limit. Each of the four CHSH terms contributes
\(\pm(1/\sqrt{2})\), so:
...
\subsubsection{3.5 Tetrahedral Orbital Geometry
(Strong)}\label{tetrahedral-orbital-geometry-strong}
...
the tetrahedral angle of 109.47°. This angle is not an input parameter;
...
Water's H-O-H bond angle of 104.5° represents the same tetrahedral
electron geometry compressed by the asymmetric repulsion of two lone
electron pairs versus two bonding pairs. The deviation from 109.47°
(approximately 5°) is the measurable signature of the D-C imbalance
introduced by the higher repulsion weight of lone pairs relative to
bonding pairs.

Methane (CH₄), silane (SiH₄), and the diamond crystal lattice all
exhibit tetrahedral geometry from the same electron-pair repulsion
minimization principle.

\subsubsection{3.6 Electron Spin as Binary N-D Duality
(Moderate)}\label{electron-spin-as-binary-n-d-duality-moderate}
...
the tholonic ladder
(\href{https://github.com/baardev/truevalue/blob/main/docnav/Research/papers/1_recursive-tholonic-five-constants/1_recursive-tholonic-five-constants.pdf}{paper
1 in this series}), where the \(\ln 2\) branch carries an offset of
\(+1/2\) structurally necessary for its convergence, the midpoint
between the 0 and 1 states, the boundary between the N and D poles.
Electron spin \(\pm 1/2\) occupies a structurally analogous position.
...
\subsubsection{3.7 The Five Tholonic Constants: A Structural Unification
Argument (Moderate,
Qualified)}\label{the-five-tholonic-constants-a-structural-unification-argument-moderate-qualified}

\href{https://github.com/baardev/truevalue/blob/main/docnav/Research/papers/1_recursive-tholonic-five-constants/1_recursive-tholonic-five-constants.pdf}{Paper
1 in this series} proves that five classical mathematical constants
...
than abstract role assignments.
\href{https://github.com/baardev/truevalue/blob/main/docnav/Research/papers/1_recursive-tholonic-five-constants/1_recursive-tholonic-five-constants.pdf}{Paper
1} establishes that the seeds \(D_0 = 3\) and \(C_0 = 5\) of the
\(\pi/4\) branch are the Contribution and Definition axis multipliers of
the thologram respectively (Pattern 2 of Section 2.3). The step size
\(\Delta = 4\) is derived from squaring the Instantiation axis
multiplier (\(2^2 = 4\)). The five constants therefore emerge from the
specific axis geometry of the thologram, not from arbitrary parameter
choices. Since \(\pi\) is among those constants and \(\pi\) appears in
the fundamental equations of atomic wave functions (the Bohr radius, all
spherical harmonics), the connection is between the thologram's axis
structure and the wave geometry of quantum mechanics. The axis
multipliers (2, 3, 5) are the generative source of the same \(\pi\) that
...
\subsubsection{3.8 The Koide Formula and Three-Generation Structure
(Strong)}\label{the-koide-formula-and-three-generation-structure-strong}
...
structural derivation (showing that the specific quadratic mean form of
the Koide formula follows from tholonic role weights) has not yet been
produced. Until that derivation exists, this is a suggestive numerical
correspondence rather than a structural prediction. It is nonetheless
classified Strong because: (a) the Standard Model is entirely silent on
this value, (b) the tholonic model generates 2/3 from its structure, and
(c) the quark-triplet extension is an immediately testable prediction.
Preliminary computation of \(Q\) for the quark triplets (Section 8.1)
yields values substantially above 2/3, which the tholonic model must
address: either quark mass running scheme dependence accounts for the
deviation, or the lepton sector is uniquely tholonic while quarks
require a different mapping, or the structural derivation of the Koide
form (once produced) will constrain which triplets are expected to
satisfy \(Q = 2/3\).
...
fermion generations.
\href{https://github.com/baardev/truevalue/blob/main/docnav/Research/papers/3_minimal-recursive-triadic-framework/3_minimal-recursive-triadic-framework.pdf}{Paper
3 in this series} proves that three roles are the minimum for
non-trivial recursive stability. If this requirement applies at the
generation level, three generations is a structural prediction of the
tholonic model, not an observed coincidence to be accommodated after the
fact. No fourth-generation fermion has been found, which is consistent
with this prediction.

\begin{center}\rule{0.5\linewidth}{0.5pt}\end{center}

\subsection{4. The N-D-C Assignment for the Hydrogen
Atom}\label{the-n-d-c-assignment-for-the-hydrogen-atom}
...
\subsection{5. Evidence Strength
Summary}\label{evidence-strength-summary}
...
& Strong & Sign pattern derived from T1+T2+T3 (T3 proved from
\href{https://github.com/baardev/truevalue/blob/main/docnav/Research/papers/3_minimal-recursive-triadic-framework/3_minimal-recursive-triadic-framework.pdf}{Paper
3}, Lemma 4.2). Value \(2\sqrt{2}\) derived from real-virtual pair
...
\subsection{6. The Standard Model's Parameter
Problem}\label{the-standard-models-parameter-problem}
...
requires exactly three roles (N, D, C), proved in
\href{https://github.com/baardev/truevalue/blob/main/docnav/Research/papers/3_minimal-recursive-triadic-framework/3_minimal-recursive-triadic-framework.pdf}{Paper
3}. If the three quark colors, three fermion generations, and three-role
quark composition of hadrons are all structural consequences of this
...
Structural: three roles are the minimum for stable recursion
(\href{https://github.com/baardev/truevalue/blob/main/docnav/Research/papers/3_minimal-recursive-triadic-framework/3_minimal-recursive-triadic-framework.pdf}{Paper
3}) & High \\
...
\subsection{8. Proposed Tests and Open
Derivations}\label{proposed-tests-and-open-derivations}
...
axes are oriented at tetrahedral angles (\(\arccos(1/3) \approx 70.53°\)
...
balance angle and the \(\pi\)-branch seed from
\href{https://github.com/baardev/truevalue/blob/main/docnav/Research/papers/1_recursive-tholonic-five-constants/1_recursive-tholonic-five-constants.pdf}{Paper
1}), \(\cos(\pi/4) = 1/\sqrt{2}\) is the tholonic convergence limit, and
\(\mathrm{CHSH}_{\max} = 4\cos(\pi/4) = 2\sqrt{2}\). The sign pattern is
derived from T1+T2+T3, where T3 (D\(\times\)C balance sign rule) is
itself derived from
\href{https://github.com/baardev/truevalue/blob/main/docnav/Research/papers/3_minimal-recursive-triadic-framework/3_minimal-recursive-triadic-framework.pdf}{Paper
3's} Lemma 4.2 and Definition 4.3 without reference to measurement
scenarios. A companion computational script
...
\textbf{Why exactly three fermion generations?}
\href{https://github.com/baardev/truevalue/blob/main/docnav/Research/papers/3_minimal-recursive-triadic-framework/3_minimal-recursive-triadic-framework.pdf}{Paper
3} proves three roles are the minimum for non-trivial recursive
stability. If this requirement applies at the generation level, three
generations is a structural prediction. No fourth-generation fermion has
been found, which is consistent with this prediction but not
specifically predicted by standard QM.
...
\subsection{9. Implications}\label{implications}
...
\item
  \textbf{\href{https://github.com/baardev/truevalue/blob/main/docnav/Research/papers/1_recursive-tholonic-five-constants/1_recursive-tholonic-five-constants.pdf}{Paper
  1 in this series}:} \emph{Emergence of Classical Constants from a
  Minimal Recursive Triadic Framework}: derivation of \(\pi/4\),
  \(\phi\), \(e\), \(\sqrt{2}\), and \(\ln 2\) from tholonic recursion.
\item
  \textbf{\href{https://github.com/baardev/truevalue/blob/main/docnav/Research/papers/3_minimal-recursive-triadic-framework/3_minimal-recursive-triadic-framework.pdf}{Paper
  3 in this series}:} \emph{Minimal Recursive Triadic Framework}: formal
  proof that three functional roles are the minimum for non-trivial
