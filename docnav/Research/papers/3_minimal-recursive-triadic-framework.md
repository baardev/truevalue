...
3. **Triadic partition** ($\S 4$). A recurrence on $m$ real variables with a distinguished state variable and two functionally independent auxiliary variables requires $m \ge 3$ (Lemma 4.1). The three roles correspond to the three axes of the triangular geometry: bounding (Definition axis), accumulating (Contribution axis), and instantiating (Instantiation axis).
