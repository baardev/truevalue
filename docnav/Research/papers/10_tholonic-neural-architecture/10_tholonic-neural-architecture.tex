...
consistent across all 18 architecture families tested, is not explained
by architectural variation, and is correlated with the one variable that
distinguishes the outlier model (Phi-3-mini, the only model with
non-trivial virial balance, trained with minimal post-hoc RLHF). The
paper argues that this pattern is the structural fingerprint of
post-training C-dominant intervention: every current alignment procedure
(RLHF, Constitutional AI, safety fine-tuning) adds integrative capacity
without a corresponding increase in definitional constraint,
systematically inflating C relative to D in precisely the interior zones
where the framework predicts self-sustaining balance. A causal training
experiment confirms that adding the virial balance regulariser during
training improves convergence speed with a growing advantage (0.153 nat
...
Qwen2.5-0.5B & Qwen & 86 & \textbf{97} & 75 & 0 & 57 \\
...
ln2 compression, virial balance, and composite). The all-but-empty
virial column is the dominant feature: no current architecture
approaches the D/C = 0.5 equilibrium target.}
\end{figure}

\textbf{The virial bottleneck.} The most striking result is structural
rather than phase-based: 18 of 20 models score 0 on the virial-balance
axis. All architectures are strongly C-dominant; measured D/C ratios
...
The one meaningful outlier is Phi-3-mini (virial = 82, D/C = 0.591),
which achieves the highest overall structural health score in the
20-model set (66). Phi-3-mini was trained primarily on synthetic data
with substantially less post-hoc RLHF than comparably sized
instruction-tuned models. This is consistent with the post-training
intervention interpretation developed in Section 13.3c: less C-dominant
post-training correction produces a more structurally balanced model.
The correlation between training procedure and virial balance, if it
holds under broader sampling, would be the clearest evidence that
current alignment procedures are the proximate cause of universal
C-dominance, rather than the architectural choices themselves.
...
Eighteen of twenty score zero; Phi-3-mini (82) is the only model
approaching the D/C = 0.5 virial equilibrium target. Measured D/C ratios
across all models fall between approximately 0.02 and 0.59.}
\end{figure}
...
relationship with architecture quality. GPT-Neo 125m and OPT 125m each
achieve ln2 compression scores of 97, indicating near-perfect tholonic
governance of the output-projection stage. Qwen2.5-0.5B achieves the
highest \(\phi\)-equilibrium score (97), suggesting that its mid-network
has the strongest self-similar stability of any model tested. More
modern architectures (Qwen, GPT-Neo, OPT) consistently outscore older
ones (GPT-1, distilGPT-2) on phase-structure axes, consistent with the
...
\(\text{decay}(l) = 1 - \overline{s_{l+2:L}/s_{l+1}}\). The pass rate
across 14 models was 43\% (6/14, same as the context-length test).
...
zones:
\(\overline{h(\phi\text{-zone})} < \overline{h(\text{outside})}\). The
pass rate across 14 models was \textbf{14/14 = 100\%}, for both noise
types independently and for the joint criterion.
...
not prevent residual leakage at later layers.

The 100\% pass rate on Experiment 3.2 is the strongest single-experiment
evidence obtained to date. It is consistent with the tholonic prediction
that the \(\phi\)-zone is a genuine self-stabilizing attractor at every
scale of the network hierarchy. The fact that this property holds even
in GPT-2, whose interior zone balance is among the most disrupted (all
GPT-2 models fail the boundary fidelity axis of the health assessment),
...
directionally consistent across all 18 architecture families tested, and
is not explained by architectural variation. The one model that departs
from this pattern, Phi-3-mini, trained on synthetic data with minimal
post-hoc RLHF, also has the highest virial balance score (82) of any
model tested. Third, a causal training experiment confirms that adding
the virial regulariser during training improves validation loss by 0.153
nats at 10,000 steps compared to a matched baseline, with the advantage
widening throughout training.
