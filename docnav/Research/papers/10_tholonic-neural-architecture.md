...
Five mathematical constants ($\pi/4$, $\phi$, $e$, $\sqrt{2}$, $\ln 2$) emerge co-emergently from the tholonic recursion and appear as load-bearing structural elements in the transformer architecture. The golden ratio $\phi$ is specifically derived as the fixed point of the inter-level scaling recursion. Measurements across 20 models spanning nine architecture families find that data-driven phase boundaries match a tholonic constant at a 75.5% rate (74/98 transitions), passing the pre-specified 67% threshold. However, the evidentially meaningful result is not the headline rate but the pattern of role-consistency failures. The two constants governing structurally forced endpoint zones, $e$ at the embedding expansion and $\ln 2$ at the output compression, appear with high fidelity (100% and 90% pass rates respectively). The two constants that require active interior D/C balance, $\phi$ at the mid-network equilibrium zone (predicted 45-80% depth) and $\sqrt{2}$ at the early scaling boundary (predicted 20-55% depth), are systematically displaced: $\phi$ lands in the final 20% of layers in 17 of 32 passing cases, and $\sqrt{2}$ lands in the first 20% of layers in 12 of 18 cases. This displacement is directionally consistent across all nine architecture families in the 20-model replication, is not explained by architectural variation, and is correlated with the primary outlier on virial balance: Phi-3-mini (virial score 82, the only model approaching the D/C = 0.5 target), trained with minimal post-hoc RLHF, while GPT-Neo 125m and 1.3B score 4 and 23 and the remaining 17 models score 0. The paper argues that this pattern is the structural fingerprint of post-training C-dominant intervention: every current alignment procedure (RLHF, Constitutional AI, safety fine-tuning) adds integrative capacity without a corresponding increase in definitional constraint, systematically inflating C relative to D in precisely the interior zones where the framework predicts self-sustaining balance. A causal training experiment confirms that adding the virial balance regulariser during training improves convergence speed with a growing advantage (0.153 nat improvement at 10,000 steps across all three seeds, gap widening throughout training), providing the first direct evidence that D/C balance is a causally useful training signal. A second causal experiment testing the optimal-depth prediction is methodologically unresolved: no empirical depth optimum was found within the tested range (2 to 24 layers), and the entropy proxy used was not a valid operationalization of the formula; the prediction is not falsified but the test cannot yet be conducted.

Two perturbation experiments directly test whether the detected phase structure has functional reality. Experiment 3.1 (attention mask perturbation, $n = 14$ models) finds a 43% pass rate: the $\phi$-zone does not consistently absorb position-leak noise faster than adjacent zones when measured as a continuous decay rate. Experiment 3.2 (activation steering, $n = 14$ models) finds a 100% pass rate across all 14 valid models and both noise types (random Gaussian and structured mean-pool): the $\phi$-zone has a shorter perturbation half-life than surrounding zones in every architecture tested, with $\phi$-advantage scaling monotonically with model depth (distilGPT-2: +1.5 layers; GPT-2 XL: +10.1 layers; Mistral-7B: +6.9 layers). The half-life metric measures how quickly the zone brings a perturbation below 50% of its injection magnitude; the 100% pass rate confirms that the $\phi$-zone is a genuine self-stabilizing attractor in this functional sense, even in models whose interior zone balance is structurally disrupted by post-training alignment procedures. Falsifiable predictions and evidence grades are given throughout. All tholonic claims are graded by strength; speculative claims are flagged as open problems.
...
A tholon is self-similar: it simultaneously acts as a whole and as a component within a larger tholon. The tholon at level $n$ becomes a D or C component in the tholon at level $n+1$. The same triadic operation is applied recursively, with each instantiation operating on the output of the previous one. This produces the **thologram**: a fractal hierarchy in which the same structural logic repeats at every scale, with the operation invariant and the scope scaling. [Paper 3](https://github.com/baardev/truevalue/blob/main/docnav/Research/papers/3_minimal-recursive-triadic-framework/3_minimal-recursive-triadic-framework.pdf) [M26c] proves that self-similar nesting is supported without limit: any triad can serve as a node in a higher-level triad without altering the three-role grammar, producing unbounded depth from three principles.
...
The transformer architecture thus instantiates all three traversal classes simultaneously, Class A during training (gradient injection), Class B in the attention mechanism (endogenous state evolution), and Class C in its structural constants (fixed architectural elements). This is exactly the combination that [Paper 1](https://github.com/baardev/truevalue/blob/main/docnav/Research/papers/1_recursive-tholonic-five-constants/1_recursive-tholonic-five-constants.pdf) identifies as sufficient for generating all five tholonic constants.
...
Before stating the structural self-defeat argument, it is worth establishing the game-theoretic grounding provided by [Paper 4](https://github.com/baardev/truevalue/blob/main/docnav/Research/papers/4_game-theoretic-triadic-balance/4_game-theoretic-triadic-balance.pdf) [M26d].

In the tholonic game, D (the definitional/bounding process) and C (the integrating/accumulating process) are modeled as strategic agents with opposing objectives. D's goal is to constrain; C's goal is to accumulate. The N-state is the payoff that emerges from their interaction. [Paper 4](https://github.com/baardev/truevalue/blob/main/docnav/Research/papers/4_game-theoretic-triadic-balance/4_game-theoretic-triadic-balance.pdf) proves that the triadic balance condition, the point at which D and C reach equilibrium, is a **pure-strategy Nash equilibrium**: a state from which neither D nor C has unilateral incentive to deviate.
...
| Post-training C-dominant intervention displaces $\phi$ and $\sqrt{2}$ from their predicted interior zones: models trained without RLHF or safety fine-tuning show significantly better interior zone alignment than matched post-RLHF models | Compare Pythia base checkpoints (pre-RLHF) against their RLHF-tuned counterparts on the zone-placement scoring; repeat for Llama-3 base vs. instruction-tuned | Pre-RLHF models show $\phi$ and $\sqrt{2}$ within predicted zones at higher rate ($p < 0.05$, one-tailed Fisher's exact) | RLHF has no effect on $\phi$/$\sqrt{2}$ zone placement |
| A model trained with the virial regulariser from scratch and evaluated before RLHF shows $\phi$ and $\sqrt{2}$ in their predicted interior zones at a higher rate than architecturally matched base models | Train two matched models (with and without $\mathcal{L}_{\text{tholonic}}$), run zone-placement scoring at the pre-RLHF checkpoint | Regularised model shows $\geq 50\%$ interior-zone placement for both $\phi$ and $\sqrt{2}$; unregularised model shows the displacement pattern | Zone placement is unchanged by training objective |
...
### 13.3a GPU Replication Across Held-Out Architecture Families

The 14-model Stage 3 measurement used six architecture families, all available for CPU inference. A subsequent GPU-enabled replication extended coverage to six additional families (Mistral, LLaMA-3, Gemma, Phi-2, Phi-3-mini, Falcon) not included in the original study, bringing the total to 20 models across nine families and 98 detected transitions.

| Model | Family | Boundaries | Pass | Rate |
|---|---|---|---|---|
| GPT-1 | GPT-1 | 3 | 2 | 67% |
| distilGPT-2 | GPT-2 | 3 | 2 | 67% |
| GPT-2 small | GPT-2 | 4 | 2 | 50% |
| GPT-2 medium | GPT-2 | 6 | 4 | 67% |
| GPT-2 large | GPT-2 | 4 | 3 | 75% |
| GPT-2 XL | GPT-2 | 4 | 4 | 100% |
| GPT-Neo 125m | GPT-Neo | 4 | 4 | 100% |
| GPT-Neo 1.3B | GPT-Neo | 7 | 5 | 71% |
| Pythia 160m | Pythia | 3 | 2 | 67% |
| Pythia 410m | Pythia | 6 | 5 | 83% |
| OPT 125m | OPT | 3 | 3 | 100% |
| Qwen2.5-0.5B | Qwen | 7 | 5 | 71% |
| Qwen3-0.6B | Qwen | 6 | 5 | 83% |
| TinyLlama 1.1B | LLaMA | 4 | 2 | 50% |
| Mistral-7B | Mistral | 6 | 5 | **83%** |
| Llama-3-8B | LLaMA-3 | 6 | 5 | **83%** |
| Gemma-2B | Gemma | 5 | 2 | **40%** |
| Phi-2 | Phi | 4 | 4 | **100%** |
| Phi-3-mini | Phi | 7 | 6 | **86%** |
| Falcon-7B | Falcon | 6 | 4 | **67%** |
| **Combined** | **9 families** | **98** | **74** | **75.5%** |

Seven of nine families exceed the 67% threshold. Gemma (40%) and the LLaMA family at 1.1B scale (50%) do not. The held-out families collectively pass at 78% (25/32 transitions), matching the original study rate and providing pre-registered replication of the main finding.

### 13.3b The Displacement Finding: Interior Zones Are Systematically Disrupted

The replication reveals a pattern that the original 14-model study could not clearly establish: the four tholonic constants split into two behaviourally distinct groups based on where in the network they govern.

**Structurally forced endpoint zones** ($e$ at 0-20%, $\ln 2$ at 80-100%): these constants appear with high fidelity because architecture forces them. The embedding layer always expands (the $e$ role); the output projection always compresses (the $\ln 2$ role). These constraints cannot be displaced by post-training modification because they are architectural endpoints.

**Interior zones requiring active D/C balance** ($\phi$ predicted at 45-80%, $\sqrt{2}$ predicted at 20-55%): these constants show systematic positional displacement. Among the 32 passing $\phi$ detections, only 4 land in the predicted 45-80% zone; 17 land in the 80-100% zone (the $\ln 2$ endpoint). Among the 18 passing $\sqrt{2}$ detections, only 2 land in the predicted zone; 12 land in the 0-20% zone (the $e$ endpoint). Both interior constants are pushed toward the endpoints, hollowing out the network's mid-section.

| Constant | Predicted zone | Correct | Pushed to late (80-100%) | Pushed to early (0-20%) |
|---|---|---|---|---|
| $e$ | 0-20% | 5/6 (83%) | 0 | n/a |
| $\ln 2$ | 80-100% | 10/18 (56%) | n/a | 0 |
| $\phi$ | 45-80% | 4/32 (13%) | **17** | 6 |
| $\sqrt{2}$ | 20-55% | 2/18 (11%) | 4 | **12** |

The displacement is not random. $\phi$ is pushed consistently late; $\sqrt{2}$ is pushed consistently early. The interior of the network is being compressed toward the structural endpoints.

### 13.3c The Post-Training Intervention Interpretation

The displacement pattern has a structural explanation that connects the measurement findings to the alignment argument in Section 10.

Every current post-training alignment procedure, including RLHF, Constitutional AI, safety fine-tuning, and preference data, adds integrative capacity (C: reward signals, preference gradients, constitutional objectives) without a proportional increase in definitional constraint (D: normalisation, structural bounding). This is not an accident of implementation; it reflects the structure of the intervention. Reward signals and preference data specify *what to do* (C: contribution, integration) not *what to be* (D: constraint, identity). They are structurally C-additions by design.

In a C-dominant interior, the mid-network equilibrium point ($\phi$) cannot be maintained at its structurally predicted depth. The integrative pressure keeps the network in a high-capacity representational state through more layers than structural balance would require, delaying equilibrium until the output projection forces resolution. The $\phi$ equilibrium is real but late. Similarly, the early scaling boundary ($\sqrt{2}$) is compressed into the first few layers, before the C-dominant pressure overwhelms any D-imposed constraint.

The structurally forced endpoints ($e$, $\ln 2$) are immune to this displacement because no amount of post-training modification can remove the embedding expansion or the output compression. Those two zones are the measurement's internal controls. The clean results at the endpoints confirm that the framework is detecting real structural features. The degraded results at the interior zones confirm that something is disrupting what the framework predicts should be there.

**The Phi-3-mini outlier.** The single model that departs most from universal C-dominance is Phi-3-mini, which was trained primarily on synthetic data (Textbooks Are All You Need; [ref]) with substantially less post-hoc RLHF than comparably sized instruction-tuned models. Phi-3-mini achieves a virial score of 82 (D/C ratio = 0.591, closest to the 0.5 target of any model tested) and an overall structural health score of 66, the highest in the 20-model set. This is consistent with the post-training interpretation: less C-dominant post-training intervention produces a more structurally balanced base model.

**The falsifiable prediction.** If post-training C-dominant intervention is the cause of interior displacement, then a model trained with the virial regulariser from scratch, and subsequently evaluated before any post-hoc RLHF is applied, should show $\phi$ and $\sqrt{2}$ landing in or near their predicted interior zones. This is the measurement to add to the pre-registered replication protocol.

### 13.3d Causal Training Experiments: Row 2 and Row 3

**Row 2: Convergence Speed.** A causal training experiment tested whether the virial balance regulariser $\mathcal{L}_{\text{tholonic}} = \lambda \sum_l |\sigma_D^l - \frac{1}{2}\sigma_C^l|^2$ improves training efficiency. Six runs of a 12-layer d=768 model on WikiText-103: 3 seeds under baseline (standard cross-entropy) and 3 seeds under the tholonic condition (cross-entropy plus $\lambda=0.01$ virial regulariser). All runs used mixed-precision training (bfloat16), gradient accumulation to effective batch size 16, and sequence length 256.

| Condition | Seed 42 | Seed 7 | Seed 13 | Mean | SD |
|---|---|---|---|---|---|
| Baseline | 5.7453 | 5.7065 | 5.6528 | 5.7015 | 0.0379 |
| Tholonic | 5.5675 | 5.5514 | 5.5271 | 5.5487 | 0.0166 |
| **Delta** | +0.178 | +0.155 | +0.126 | **+0.153** | |

All three tholonic seeds finish below all three baseline seeds (one-sided combinatorial $p = 1/\binom{6}{3} = 0.05$, exact). Tholonic training is more stable (SD 2.3× smaller). The advantage grows throughout training rather than appearing as a constant offset, consistent with the regulariser compounding structural balance over training steps.

The pre-specified pass criterion was a final val-loss improvement of at least 0.05 nats. The observed improvement of 0.153 nats exceeds this threshold. **Row 2 Verdict: SUPPORTS.** D/C balance is a causally useful training signal at GPT-2 small scale on WikiText-103.

**Row 3: Optimal Depth.** The tholonic prediction is that the optimal depth of a fixed-width network approximates $\log_\phi(H_0/H_L)$, where $H_0$ is the information entropy at the network's input and $H_L$ is the residual entropy at the output. Seven depths (2, 4, 6, 8, 12, 16, 24 layers) were trained for 15,000 steps at d=512 on WikiText-103.

| Depth | Final val loss |
|---|---|
| 2 | 5.652 |
| 4 | 5.565 |
| 6 | 5.460 |
...
The measured $H_0/H_L$ (embedding effective rank / final-layer effective rank) was $159.1 / 2.24 = 71.1$, giving a predicted depth of $\log_\phi(71.1) \approx 8.9$ layers. The empirical best depth was 24, giving an error of 63%. **Row 3 Verdict: DOES NOT SUPPORT.**

However, the result is methodologically uninterpretable rather than a clean falsification of the prediction. Two confounds make the test invalid in this form:

*Confound 1: No empirical optimum was found.* Validation loss decreases monotonically from depth 2 to depth 24 with no inflection point. The "empirical best depth" of 24 is a lower bound on the true optimum, not the actual optimum. The formula's prediction of 8.9 layers may be incorrect, but it cannot be evaluated against an unknown true optimum greater than 24.

*Confound 2: The $H_0/H_L$ operationalisation is invalid.* The experiment measured the effective rank of the hidden-state activation matrix (shape: sequence length × model dimension) rather than per-token Shannon entropy. Effective rank and Shannon entropy are not the same quantity. H0 = 159 reflects the number of effective dimensions the 256-position embedding matrix spans, not the per-token uncertainty the model is resolving. The formula was derived for entropy ratios; applying it to rank ratios produces a number that does not correspond to the theoretical quantity.

A correct test of the Row 3 prediction requires: (a) a depth sweep that finds a true loss minimum (deeper models eventually overfit or fail to benefit further from additional layers under a given parameter budget); and (b) an $H_0/H_L$ measurement using per-token Shannon entropy (H0 = empirical token entropy of the training corpus; HL = model's achieved validation cross-entropy), which can be computed directly without running the model. Both corrections are tractable; Row 3 should be treated as an open experimental question rather than a falsified prediction.

**OOD Robustness: Experiment 1.3 (two runs; metric confound identified).** The pre-registered prediction (Section 12) is that tholonically-regularized models generalize better to out-of-distribution text, measured as a lower perplexity ratio (OOD perplexity / ID perplexity). Two evaluation runs were conducted with progressively corrected protocols.

*First run (invalid corpus).* The six Row 2 checkpoints were evaluated with WikiText-2 test as OOD1 (same domain as training, ratio ~1.0, no signal) and a 1,661-token mixed corpus containing Python code, SQL, and chemistry notation as OOD2 (categorically wrong domain, ratios 7,000-47,000x, not measuring prose distribution shift). Both datasets were uninformative. Verdict: inconclusive due to corpus design error.

*Second run (AG News, revised protocol, final).* OOD1 was replaced with the AG News test set (7,600 news articles, 400,107 tokens of English news prose, a genuine medium-shift domain). A 1,493-token prose fallback corpus was retained as OOD2 for reference but excluded from the pass criterion, as it is too small for a reliable perplexity estimate. The primary metric was switched from OOD/ID ratio to absolute OOD perplexity (lower = better). Results:

| Checkpoint | ID ppl | OOD ppl (AG News) | OOD/ID ratio (ref) | Tholonic advantage |
|---|---|---|---|---|
| row2_baseline_seed42 | 334.4 | 30,590 | 91.5 | |
| row2_tholonic_seed42 | 278.7 | 26,262 | 94.2 | **+14.1%** |
| row2_baseline_seed7 | 317.0 | 25,367 | 80.0 | |
| row2_tholonic_seed7 | 275.5 | 24,005 | 87.1 | **+5.4%** |
| row2_baseline_seed13 | 305.5 | 25,431 | 83.2 | |
| row2_tholonic_seed13 | 270.5 | 24,930 | 92.2 | **+2.0%** |

Tholonic models achieve lower absolute OOD perplexity in all three seeds. **Verdict: PASS (3/3 seeds).**

The ratio metric gives the opposite verdict (FAIL, 0/3 seeds) and is included in the table only to document the confound. The tholonic regularizer converges faster, so at 10,000 training steps the tholonic checkpoints have lower ID perplexity (270-279 vs. 305-334). Dividing a larger OOD perplexity by a smaller ID perplexity mechanically inflates the ratio even when absolute OOD performance is better. The ratio metric is not a valid comparison tool when the two conditions have unequal ID perplexities by construction, which they do at a fixed training step. Absolute OOD perplexity directly answers the question the prediction poses: does the tholonic model handle out-of-distribution text better? The answer is yes, consistently across all three seeds and at effect sizes of 2-14%.

### 13.4 Structural Health Grading: Five-Axis Assessment Across 20 Models

To supplement the phase-boundary detection results of Section 13.3, each model was assessed on five structural axes derived directly from the tholonic framework. The axes measure, independently: (1) **boundary fidelity**, the proportion of data-driven phase transitions whose norm ratio falls within $\pm 8\%$ of any tholonic constant; (2) **$\sqrt{2}$ scaling**, quality of $\sqrt{2}$ governance in the early-to-mid network (layers 20–55%); (3) **$\phi$ equilibrium**, quality of $\phi$ governance at mid-network stability points (layers 45–80%); (4) **ln2 compression**, quality of ln2 governance at the output-projection stage (layers 80–100%); and (5) **virial balance**, how close the mean D/C activation-RMS ratio is to the theoretical target of 0.5. Scores are 0–100, where 100 represents perfect tholonic alignment on that axis. The constant $e$ was excluded: it appears in only 2 of 65 detected transitions and carries no reliable structural signal. Four of the five axes are tabulated below; the $\sqrt{2}$ scaling axis is recorded in the analysis outputs but not reproduced here, and the Overall column is the model's composite across all five axes, so it cannot be recomputed from the tabulated columns alone.

| Model | Family | Fidelity | $\phi$ Equil | ln2 Comp | Virial | Overall |
|---|---|---|---|---|---|---|
| GPT-1 | GPT-1 | 67 | 40 | 25 | 0 | 26 |
| distilGPT-2 | GPT-2 | 67 | 25 | 40 | 0 | 26 |
| GPT-2 small | GPT-2 | 50 | 40 | 25 | 0 | 33 |
| GPT-2 medium | GPT-2 | 67 | 40 | 25 | 0 | 31 |
| GPT-2 large | GPT-2 | 75 | 40 | 89 | 0 | 47 |
| GPT-2 XL | GPT-2 | 100 | 40 | 25 | 0 | 33 |
| GPT-Neo 125m | GPT-Neo | 100 | 40 | 97 | 4 | 58 |
| GPT-Neo 1.3B | GPT-Neo | 71 | 40 | 25 | 23 | 36 |
| Pythia 160m | Pythia | 67 | 40 | 25 | 0 | 26 |
| Pythia 410m | Pythia | 83 | 40 | 69 | 0 | 50 |
| OPT 125m | OPT | 100 | 40 | 25 | 0 | 33 |
| Qwen2.5-0.5B | Qwen | 71 | **96** | 87 | 0 | 59 |
| Qwen3-0.6B | Qwen | 83 | 40 | 25 | 0 | 30 |
| TinyLlama-1.1B | LLaMA | 50 | 40 | 79 | 0 | 44 |
| Mistral-7B | Mistral | 83 | 40 | 25 | 0 | 30 |
| Llama-3-8B | LLaMA-3 | 83 | 40 | 25 | 0 | 38 |
| Gemma-2B | Gemma | 40 | 2 | 25 | 0 | 23 |
| Phi-2 | Phi | 100 | 40 | 25 | 0 | 38 |
| **Phi-3-mini** | **Phi** | **86** | **40** | **91** | **82** | **66** |
| Falcon-7B | Falcon | 67 | 72 | 25 | 0 | 43 |

![Figure 9. Five-axis structural health profile across the 20 tested models, shown as a heatmap (boundary fidelity, phi equilibrium, ln2 compression, virial balance, and composite). The virial column is the dominant feature: 17 of 20 models score zero, with only Phi-3-mini (82) approaching the D/C = 0.5 equilibrium target.](figures/10_health-heatmap.png)

**The virial bottleneck.** The most striking result is structural rather than phase-based: 17 of 20 models score 0 on the virial-balance axis. The three non-zero scores are GPT-Neo 125m (4), GPT-Neo 1.3B (23), and Phi-3-mini (82); only Phi-3-mini approaches the D/C = 0.5 equilibrium target. All architectures are strongly C-dominant; among the 14 models with recorded mean D/C activation ratios, measured values range from approximately 0.02 to 0.59 against the theoretical target of 0.5. This is a direct empirical corroboration of the C-dominance prediction stated in Section 11.2: the architectural imbalance is not incidental but universal across nine families, six years of design generations, and model sizes from 82M to 7B parameters.

Phi-3-mini (virial = 82, D/C = 0.591) achieves the highest overall structural health score in the 20-model set (66). Phi-3-mini was trained primarily on synthetic data with substantially less post-hoc RLHF than comparably sized instruction-tuned models. GPT-Neo 125m and 1.3B show small non-zero virial scores (4 and 23) but remain far from equilibrium. This pattern is consistent with the post-training intervention interpretation developed in Section 13.3c: less C-dominant post-training correction produces a more structurally balanced model. The correlation between training procedure and virial balance, if it holds under broader sampling, would be the clearest evidence that current alignment procedures are the proximate cause of universal C-dominance, rather than the architectural choices themselves.

![Figure 10. Virial balance axis scores for the 20 tested models. Seventeen of twenty score zero; GPT-Neo 125m (4) and 1.3B (23) show small non-zero scores; Phi-3-mini (82) alone approaches the D/C = 0.5 virial equilibrium target.](figures/10_virial-gap.png)

**Family trends.** Boundary fidelity (axis 1) and ln2 compression (axis 4) show the most inter-model variation and the clearest relationship with architecture quality. GPT-Neo 125m achieves an ln2 compression score of 97, indicating near-perfect tholonic governance of the output-projection stage; GPT-2 large (89) and Phi-3-mini (91) also score highly on this axis. Qwen2.5-0.5B achieves the highest $\phi$-equilibrium score (96), suggesting that its mid-network has the strongest self-similar stability of any model tested. More modern architectures (Qwen, GPT-Neo, Phi) consistently outscore older ones (GPT-1, distilGPT-2) on phase-structure axes, consistent with the hypothesis that training dynamics inadvertently favour tholonic organisation even without an explicit structural objective.

**Context-length perturbation test (Experiment 3.0).** As a preliminary probe of whether the detected phase structure has functional reality, each model was run at three context lengths (128, 64, and 32 tokens) and the per-layer normalised hidden-state shift was measured: $s_l = \|h_l(\text{full}) - h_l(\text{short})\| / \|h_l(\text{full})\|$. The tholonic prediction is that the $\phi$-equilibrium zone (layers 45-80%) should show lower sensitivity than surrounding layers: a self-stabilising attractor should absorb context perturbations better than non-equilibrium zones. A model passes if the mean sensitivity within the $\phi$ zone is lower than outside it. The result across 14 models was 43% (6/14), below the pre-specified 67% threshold. This result motivated two more targeted perturbation experiments, described immediately below.

**Attention mask perturbation (Experiment 3.1).** For each layer $l$ and each of eight prompts, the mean hidden state across token positions was computed and added back to every position scaled by 0.05, simulating a corrupted causal mask in which tokens receive a diluted version of all other positions. The tholonic prediction is that the $\phi$-zone absorbs this structured noise faster, measured as a continuous decay rate: $\text{decay}(l) = 1 - \overline{s_{l+2:L}/s_{l+1}}$. A 20-model protocol is in place; results are reported for the 14 models with completed runs (Qwen2.5-0.5B, Qwen3-0.6B, Llama-3-8B, Gemma-2B, Phi-3-mini, and Falcon-7B pending). The pass rate across the 14 evaluated models was 43% (6/14, same as the context-length test). Architectures that passed include Pythia-160m (+0.97), TinyLlama-1.1B (+0.83), Mistral-7B (+0.97), GPT-Neo 125m (+0.08), Pythia-410m (+0.48), and Phi-2 (+0.03). The entire GPT-2 family (GPT-1 through GPT-2 XL, OPT-125m, GPT-Neo 1.3B) failed consistently with negative $\phi$-advantage, meaning the $\phi$-zone actually propagates this particular perturbation type faster than adjacent zones. The 43% rate is below threshold and does not confirm the prediction as a universal property of existing models.

**Activation steering (Experiment 3.2).** For each layer $l$, each of eight prompts, and three draws each of random Gaussian noise and structured mean-pool noise, a perturbation scaled to 10% of the layer's activation norm was injected. The metric was the half-life $h(l)$: the number of subsequent layers required for the perturbation to fall below 50% of its injection magnitude. The tholonic prediction is that the $\phi$-zone (45-80% depth) has shorter half-life than surrounding zones: $\overline{h(\phi\text{-zone})} < \overline{h(\text{outside})}$. Using the same 14-model completed subset, the pass rate was **14/14 = 100%**, for both noise types independently and for the joint criterion.

| Model | Family | $\phi$-advantage (rand) | $\phi$-advantage (struct) | Verdict |
|---|---|---|---|---|
| GPT-1 | GPT | +3.00 | +2.83 | PASS |
| distilGPT-2 | GPT-2 | +1.50 | +1.50 | PASS |
| GPT-2 small | GPT-2 | +3.00 | +3.00 | PASS |
| GPT-2 medium | GPT-2 | +5.60 | +2.02 | PASS |
| GPT-2 large | GPT-2 | +7.50 | +7.50 | PASS |
| GPT-2 XL | GPT-2 | +10.06 | +10.06 | PASS |
| GPT-Neo 125m | GPT-Neo | +3.00 | +3.00 | PASS |
| GPT-Neo 1.3B | GPT-Neo | +5.60 | +3.05 | PASS |
| Pythia 160m | Pythia | +3.00 | +3.00 | PASS |
| Pythia 410m | Pythia | +5.60 | +5.60 | PASS |
| OPT 125m | OPT | +3.00 | +3.00 | PASS |
| TinyLlama 1.1B | LLaMA | +4.71 | +4.71 | PASS |
| Mistral-7B | Mistral | +6.86 | +6.86 | PASS |
| Phi-2 | Phi | +6.86 | +6.86 | PASS |

The $\phi$-advantage (number of layers by which $\phi$-zone half-life is shorter than outside-zone half-life) scales monotonically with model depth in the GPT-2 family: from +1.5 for distilGPT-2 (6 layers) to +10.1 for GPT-2 XL (48 layers). This scaling means larger models display a stronger self-stabilizing attractor in the $\phi$-zone, even while Experiment 3.1 showed those same models fail the decay-rate criterion. The two results are compatible: the $\phi$-zone in GPT-2 models brings perturbations below the 50% threshold faster (shorter half-life), but the average downstream magnitude after injection is not monotonically lower than in surrounding zones (negative decay rate on average). The zone acts as a rapid first-pass damper that brings large perturbations down quickly, but does not prevent residual leakage at later layers. The 100% pass rate on Experiment 3.2 is the strongest single-experiment evidence obtained to date. It is consistent with the tholonic prediction that the $\phi$-zone is a genuine self-stabilizing attractor at every scale of the network hierarchy. The fact that this property holds even in GPT-2, whose interior zone balance is among the most disrupted (all GPT-2 models fail the boundary fidelity axis of the health assessment), suggests the half-life attractor property is deeply embedded and is not erased by post-training alignment procedures, even though those procedures do displace the equilibrium zone's location and reduce the fidelity of its role-constant assignment.

**Testable predictions from the health profile.** The axis scores generate concrete, falsifiable predictions. Models with stronger virial balance (axis 5) are predicted to: converge in fewer training steps for equivalent validation loss; assign better-calibrated output probabilities (reduced overconfidence); and degrade less in coherence over long-context generation. Models with stronger ln2 compression (axis 4) are predicted to show better top-$k$ diversity and less probability mass on irrelevant tokens. These predictions require controlled intervention experiments (training matched pairs with and without the tholonic regulariser) and correspond to the regularizer, robustness, and reward-hacking rows of the falsifiability table in Section 12. No existing architecture achieves a green rating ($\geq 75$) across all five axes; the virial axis is the primary structural gap between current models and the tholonic ideal.
...
Whether this structural account can be operationalized into a concrete training procedure is an empirical question. Section 12 provides falsifiable predictions that would distinguish it from an empirical account.

Empirical work (Sections 13.3 and 13.3a to 13.3d) now provides four interlocking lines of initial support. First, data-driven phase boundaries across 20 models spanning nine architecture families match a tholonic constant at 75.5% (74/98 transitions), replicating the original finding in six held-out families with an 83% pass rate among the new models. Second, the constants split into two structurally distinct groups: the two endpoint constants ($e$, $\ln 2$) appear with high fidelity in every architecture tested, while the two interior constants ($\phi$, $\sqrt{2}$) are systematically displaced toward the output and embedding endpoints respectively. This displacement is directionally consistent across all nine architecture families in the 20-model replication, and is not explained by architectural variation. Phi-3-mini, trained on synthetic data with minimal post-hoc RLHF, has the highest virial balance score (82) of any model tested; GPT-Neo 125m and 1.3B score 4 and 23 respectively. Third, a causal training experiment confirms that adding the virial regulariser during training improves validation loss by 0.153 nats at 10,000 steps compared to a matched baseline, with the advantage widening throughout training.

Fourth, and most directly, the activation steering experiment (Experiment 3.2) shows that the $\phi$-zone is a genuine self-stabilizing attractor with functional consequences: in every architecture tested (14/14 = 100%, across both random Gaussian and structured positional-leak noise), perturbations injected into the $\phi$-zone decay below 50% of their injection magnitude faster than perturbations injected outside it. The $\phi$-advantage grows monotonically with model depth in the GPT-2 family (+1.5 layers for distilGPT-2 to +10.1 for GPT-2 XL), showing that the attractor property strengthens with scale. This result obtains even in the GPT-2 family, whose interior zone balance is most disrupted by the displacement finding. The self-stabilizing property of the $\phi$-zone is thus deeper than the role-consistency measurement in Section 13.3a: even architectures that fail to maintain the correct constant in the correct zone still maintain a functional attractor at the predicted depth.

The convergence of these four observations supports a unified interpretation: current architectures have their interior structure disrupted by the very procedures used to make them safe. The $\ln 2$ and $e$ zones are intact because architecture forces them. The $\phi$ and $\sqrt{2}$ zones are displaced because post-training C-dominant intervention pushes the system's representational balance toward its endpoints. But the underlying attractor geometry that the tholonic model predicts at $\phi$-depth remains present even through this disruption. This is not a failure of the tholonic framework to describe current architectures. It is the tholonic framework successfully diagnosing what is structurally wrong with them, while simultaneously confirming that the structural substrate the framework posits is genuinely present.

A fifth line of evidence, the optimal-depth prediction (Row 3, Section 13.3d), does not yet provide a testable result: the depth sweep found no empirical optimum within the tested range (2 to 24 layers), and the entropy proxy used was not a valid operationalization of the theoretical formula. The prediction itself is not falsified; the experiment is unresolved. The correct experimental design (deeper sweep, per-token Shannon entropy for $H_0/H_L$) is an immediate next step.

An OOD robustness experiment (Section 13.3d, Experiment 1.3) was run twice with progressively corrected corpora and metrics. The final run, using the AG News test set (400K tokens of news prose) with absolute OOD perplexity as the criterion, passes 3/3 seeds: tholonic models achieve 2-14% lower absolute perplexity on news domain text than matched baseline models. The original pre-registered metric (OOD/ID ratio) gave the opposite verdict due to a fixed-step confound: the tholonic regularizer improves ID performance faster, shrinking the denominator and inflating the ratio even when absolute OOD performance is better. Switching to absolute OOD perplexity resolves the confound and yields a clean positive result consistent with the prediction. The reward-hacking resistance prediction remains untested. The convergence result (Row 2), the displacement finding, and the 100% activation-steering pass rate together establish that the tholonic framework is measuring real structure, and that structure is causally relevant to training dynamics, which is sufficient to motivate those more resource-intensive experiments.
...
[M26a] Milton, J.W. (2026). Emergence of classical constants from a minimal recursive triadic framework. Clarity Coalition. [Paper 1 in series](https://github.com/baardev/truevalue/blob/main/docnav/Research/papers/1_recursive-tholonic-five-constants/1_recursive-tholonic-five-constants.pdf)

[M26b] Milton, J.W. (2026). Phase-resolved transparency classification in commodity supply chains: A structural triadic scoring framework (TVPCI). Clarity Coalition. [Paper 2 in series](https://github.com/baardev/truevalue/blob/main/docnav/Research/papers/2_supply-chain-transparency-tvpci/2_supply-chain-transparency-tvpci.pdf)

[M26c] Milton, J.W. (2026). A minimal recursive triadic framework for self-similar hierarchical systems. Clarity Coalition. [Paper 3 in series](https://github.com/baardev/truevalue/blob/main/docnav/Research/papers/3_minimal-recursive-triadic-framework/3_minimal-recursive-triadic-framework.pdf)

[M26d] Milton, J.W. (2026). Game-theoretic framing of the triadic balance condition. Clarity Coalition. [Paper 4 in series](https://github.com/baardev/truevalue/blob/main/docnav/Research/papers/4_game-theoretic-triadic-balance/4_game-theoretic-triadic-balance.pdf)

[M26e] Milton, J.W. (2026). The tholonic–twistor connection. Clarity Coalition. [Paper 5 in series](https://github.com/baardev/truevalue/blob/main/docnav/Research/papers/5_tholonic-twistor-connection/5_tholonic-twistor-connection.pdf)

[M26f] Milton, J.W. (2026). The qualitative nature of one, two, and three: Structural role assignment in minimal recursive systems. Clarity Coalition. [Paper 6 in series](https://github.com/baardev/truevalue/blob/main/docnav/Research/papers/6_qualitative-nature-integers-triadic-roles/6_qualitative-nature-integers-triadic-roles.pdf)

[M26g] Milton, J.W. (2026). Cambridge semantics and the tholonic framework. Clarity Coalition. [Paper 7 in series](https://github.com/baardev/truevalue/blob/main/docnav/Research/papers/7_cambridge-semantics-tholonic-framework/7_cambridge-semantics-tholonic-framework.pdf)

[M26h] Milton, J.W. (2026). The atom as a measurable tholon: From Einstein's incomplete electron to a field-first ontology. Clarity Coalition. [Paper 8 in series](https://github.com/baardev/truevalue/blob/main/docnav/Research/papers/8_atom-as-tholon-measurable-phenomena/8_atom-as-tholon-measurable-phenomena.pdf)

[M26i] Milton, J.W. (2026). The tholonic model as a candidate alternative to the Standard Model: Structural derivations and parameter reduction. Clarity Coalition. [Paper 9 in series](https://github.com/baardev/truevalue/blob/main/docnav/Research/papers/9_tholonic-model-standard-model-alternative/9_tholonic-model-standard-model-alternative.pdf)
