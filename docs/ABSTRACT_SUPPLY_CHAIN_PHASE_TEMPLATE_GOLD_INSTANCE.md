...
This document provides the **gold supply chain instance** of the same abstract phase template used for shea butter. The full abstract template (phases 0–8, categories, N-D-C tagging, example metric types) is in **`docs/ABSTRACT_SUPPLY_CHAIN_PHASE_TEMPLATE_AND_SHEA_INSTANCE.md`** (Part I).
...
**Gold data sources (current):** `schema/supply_chain_phases.csv`, `schema/supply_chain_phases_ndc.csv`, `schema/phase_interactions_ndc.csv`, `schema/gold_supply_chain_metrics_ndc.csv` (synthetic D/C/N indices), `schema/custody_and_flow.csv` (structure only, no rows), `schema/data_sources.csv`, `docs/WATER_WASTE_METHODOLOGY.md` (benchmark ranges, not populated records). **Physical metrics** (tonnes, oz, recovery rates, custody names) are not yet populated in project data; gold_metrics CSV is empty. Exchange data (e.g. COMEX) is reference/anchor only per project rules.

---

# Part I — Abstract template (reference)

The product-agnostic template has nine phases:

- **0** Origin / resource identification  
- **1** Primary extraction / harvest  
- **2** First aggregation / bulking  
- **3** Primary transformation  
- **4** Secondary transformation / specification  
- **5** Certification / standardisation  
- **6** Logistics / custody  
- **7** Market interface / delivery  
- **8** Circular / recovery  

For each phase, categories are tagged **D**, **C**, or **N** and have example metric types. See `ABSTRACT_SUPPLY_CHAIN_PHASE_TEMPLATE_AND_SHEA_INSTANCE.md` for the full category list per phase.

---

# Part II — Gold instance (phases 0–8)
...
*Document ties to: schema/supply_chain_phases.csv, supply_chain_phases_ndc.csv, phase_interactions_ndc.csv, gold_supply_chain_metrics_ndc.csv, custody_and_flow.csv, data_sources.csv, docs/WATER_WASTE_METHODOLOGY.md.*
