...
Paths below are **relative to the `docs/` folder** in the repository (for example `Repos/intra/PDI/…` means `docs/Repos/intra/PDI/…`).
...
| [Shea instance](Activities/SHEA_SUPPLY_CHAIN_PHASE_INSTANCE.md) | Shea butter supply chain instance (current data, missing markers, example metrics) |
...
| [Supply chain rules](Guidelines/SUPPLY_CHAIN_RULES.md) | AI operating rules for the physical supply chain layer |
| [Value chain rules](Guidelines/VALUE_CHAIN_RULES.md) | Rules for value chain (profit, pricing, margins) |
...
Source files are grouped under **`docs/`** in these **top-level directories**:
...
Other Markdown and assets may appear at the **`docs/`** root or nested under the folders above. The **sidebar** follows `mkdocs.yml`; use **search** for anything not listed as a first-class page.
