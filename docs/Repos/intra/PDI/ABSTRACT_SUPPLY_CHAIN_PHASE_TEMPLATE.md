...
Product-agnostic supply chain phase model (phases 0–8) and information categories per phase, with **N-D-C tagging** and example metric types. Use this template to build instances for specific commodities (e.g. [shea butter](../../../Activities/SHEA_SUPPLY_CHAIN_PHASE_INSTANCE.md), [gold](ABSTRACT_SUPPLY_CHAIN_PHASE_TEMPLATE_GOLD_INSTANCE.md)). The **value chain** (profit, pricing, margins) has a separate [Abstract Value Chain Phase Template](ABSTRACT_VALUE_CHAIN_PHASE_TEMPLATE.md) and instances ([gold](GOLD_VALUE_CHAIN_PHASE_INSTANCE.md), [shea](SHEA_VALUE_CHAIN_PHASE_INSTANCE.md)); phases align by id only.
